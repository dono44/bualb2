...
export class DualAudioEngine {
  ...
  public async scanDevices(): Promise<AudioOutputDevice[]> {
    try {
      ...
      const formattedDevices: AudioOutputDevice[] = outputDevices.map((d, index) => {
        ...
        let displayLabel = d.label || `Périphérique Audio #${index + 1} (${d.deviceId.slice(0, 8)})`;
        if (d.deviceId === 'default') {
          displayLabel = `Périphérique Systèmes par défaut (${displayLabel})`;
        }
